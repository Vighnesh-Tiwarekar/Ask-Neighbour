...
export const create_post = async (req, res) => {
    try {
        ...
        const postimg = details.post_img || null; // Extracts URL string directly from JSON body
...
export const update_post = async (req, res) => {
    try {
        const details = req.body;
        const postimg = details.post_img === 'null' ? null : details.post_img; 
...
export const fetchposts = async (req, res) => {
    try {
        ...
        const result = await posts.aggregate([
            { $match: { user_id: { $in: userIds }, posted: true } },
            { $lookup: { from: 'profile', localField: 'user_id', foreignField: '_id', as: 'user_info' } },
            { $unwind: { path: '$user_info', preserveNullAndEmptyArrays: true } },
            {
                $project: {
                    _id: 1, title: 1, description: 1, posted: 1, post_type: 1, tags: 1, post_img: 1, updatedAt: 1,
                    'user_info.name': 1, 'user_info.contact': 1, 'user_info.email': 1, 'user_info.address': 1, 'user_info.profile_img': 1
                }
            },
            { $sort: { updatedAt: -1 } }
        ]);
        ...
        const userposts = result.map((post) => {
            // Assigns Cloudinary URLs directly 
            post.post_img_url = post.post_img ? post.post_img : null;
            post.profile_img_url = post.user_info.profile_img ? post.user_info.profile_img : null;
            post.bookmarked = post_ids?.post_ids?.includes(post._id.toString()) ? true : false;
            return post;
        });
...
export const fetchyourposts = async (req, res) => {
    try {
        ...
        const yourposts = result.map((post) => {
            // Assigns Cloudinary URL directly
            post.post_img_url = post.post_img ? post.post_img : null; 
            return post;
        });
...
export const post_status = async (req, res) => {
    try {
        const details = req.body;
        await posts.updateOne({ _id: details.post_id }, { $set: { posted: details.posted ? true : false } });
...
export const mark_post = async (req, res) => {
    try {
        const details = req.body;
        await savedposts.updateOne({ user_id: req.id }, { $addToSet: { post_ids: details.post_id } });
...
export const unmark_post = async (req, res) => {
    try {
        const details = req.body;
        await savedposts.updateOne({ user_id: req.id }, { $pull: { post_ids: details.post_id } });
